...
main_compute_npde = function(obs,sim,type_outcome="continuous",
  ...
  for(o in 1:noutcome){
    ...
    if(type_outcome[o]=="continuous"){
      ...
      if(dependancies[[o]]==0){
        npde_o = computenpde_missing_fast(obs = obs_o,sim_full = sim_full_o,compute_npde = compute_npde,
                                          compute_npd = compute_npd,decorr.method = decorr.method,verbose=verbose,ties=ties,method=method)
      }else{
        names(sim_dependancies)[1:4]=c("rep","id","tte","status")
        sim_o = merge(sim_full_o,sim_dependancies,by=c("rep","id"))
        sim_o=sim_o[time<=tte]
        
        npde_o = computenpde_missing_fast(obs = obs_o,sim = sim_o,sim_full = sim_full_o,compute_npde = compute_npde,
                                          compute_npd = compute_npd,decorr.method = decorr.method,verbose=verbose,
      ...
      if(dependancies[[o]]==0){
        npde_o <- compute_npd_discrete(obs=obs_o,sim = sim_full_o,response = "y",order_cat = order_cat)
      }else{
        
        names(sim_dependancies)[1:4]=c("rep","id","tte","status")
        sim_o = merge(sim_full_o,sim_dependancies,by=c("rep","id"))
        sim_o=sim_o[time<=tte]
        npde_o <- compute_npd_discrete(obs=obs_o,sim = sim_o,response = "y",order_cat = order_cat)
      }
    }else{
  ...
  for(o in 1:noutcome){
    ...
    if(compute_pvalue[[o]]){
      ...
      if(type_outcome[[o]]%in%c("continuous","TTE")){
        ...
        npd_sim <- compute_npd_discrete(obs=sim_o,sim = sim_o,response = "y",order_cat = order_cat)
...
main_compute_plot = function(res,type_outcome="continuous",options=NULL){
  ...
  for(o in 1:noutcome){
    ...
    npde_o = copy(res[[o]][[1]]) ## obs + npde
    sim_o = copy(res[[o]][[2]])  ## sim + VPC like
    ...
    if(length(options)>0){
      if(!is.null(options[[o]])){
        ...
        if("bin"%in%names(opt))  bin=opt[[which(names(opt)%in%"bin")]]
        if("plot.opt"%in%names(opt))  plot.opt=opt[[which(names(opt)%in%"plot.opt")]]
        if("adj"%in%names(opt))  adj=opt[[which(names(opt)%in%"adj")]]
        if("covariate"%in%names(opt))  covariate=opt[[which(names(opt)%in%"covariate")]]
        if("var"%in%names(opt))  var=opt[[which(names(opt)%in%"var")]]
        
      }
    }
    
    breaks=plot.opt$bin.breaks
    if(bin){
      if(type_outcome!="TTE"){
      npde_o$old_time=npde_o[[variable[1]]]
      bounds = auto_bin(xvec = npde_o$old_time,plot.opt=plot.opt,verbose=T)$xbound 
      if(is.null(plot.opt$bin.breaks)) breaks=bounds
      
      npde_o[[variable[1]]] = bounds
      # sim_o[[variable[1]]] = bounds
      }
    }else{
      if(is.null(plot.opt$bin.breaks)){
        if(length(unique(npde_o[[variable[[1]]]]))>10)  breaks=seq(0,max(npde_o[[variable[1]]]),length.out = 8)
        if(length(unique(npde_o[[variable[[1]]]]))<=10)  breaks=unique(npde_o[[variable[[1]]]])
      } 
    }
    ...
    if(type_outcome[[o]]%in%c("continuous")){
      ...
      if(type=="scatter"){
        ...
        myplot = plot_npde_scatter_dt_L(tab = npde_o,variable=variable,bin=bin,
                                        outcome=outcome,method_band = "theo",conf = conf)
      ...
      if(type=="scatter"){
        ...
        myplot = plot_npde_scatter_dt_TTE(tab2 = npde_o,variable = variable,outcome = outcome,adj = adj,conf = conf,covariate=covariate,var=var)
      }else if(type=="vpc"){
        
        myplot = VPC_TTE(obs = npde_o,sim = sim_o,censoring_type,var = var,bin,plot.opt)[[1]]
      }
      ...
      if(type=="scatter"){
        ...
        stop(paste0("Not implemented yet or type '",type,"' not recognised. Must be 'scatter'"))
      }
      
      # res_plot[[o]] = myplot
      
    }
    myplot = myplot+theme_pubr(legend = "right")+scale_x_continuous(name=variable[1],breaks = round(breaks))
    res_plot[[o]]=myplot
...
#### For all replicates
compute_npd_discrete = function(obs,sim,response="y",order_cat = paste0(1:3)){
...
calc_npd_discrete = function(dat,sim,response="DV1",method="unif",ties=F,order_cat = paste0(1:3)){
  ...
  sim2 = merge(sim,design_tij,by=c("id","time"))
  ...
  # sim2<-copy(sim)
  # 
  sim2[,paste0("P",order_cat):= cat_frac(y,name=order_cat),by=c("id","time")]
  
  # sim[,list(P0=cat_frac(y,name=order_cat)[[1]]),by=c("id","time")]
  
  prop_sim=unique(sim2[,c("id","time",paste0("P",order_cat)),with=F])
  ...
  # dat2[,pd_wo_ties:=pd]
  dat2[pd==0,pd:=runif(length(pd),0,1/nrep)]
  dat2[pd>=1,pd:=runif(length(pd),1-1/nrep,1)]
...
computenpde_missing_fast<-function(obs,sim=NULL,sim_full,verbose=F,compute_npde=T,
                                   compute_npd=T,decorr.method = "cholesky",ties=F,method="nall") {
  ...
  res_all = tabsim2[,list(pd=mean(ysim<y),nrep=unique(length(rep)),ypred=mean(ysim)),by=c("id","time")]
  # }
  ...
  if(compute_npde){
    ...
    if(verbose) pb = txtProgressBar(min = 0, max = length(unique(tabsim2$id)), style = 3)
    
    tabsim_decorr_same_design_as_data = tabsim2[,{
      if(verbose) {
        ID=which(unique(id)==id_vec)
        setTxtProgressBar(pb,ID)
      }
      ymat=t(correlation_matrix(tabsim_full,ID=id,nrep=nrep,decorr.method =decorr.method))
      ydsim=decorr_i(ymat,tabsim2,id,nrep,decorr.method)  ## decorr of sim
      names(ydsim)[names(ydsim)=="yd"]="ydsim"
      names(ydsim)[names(ydsim)=="y"]="ysim"
      yd=decorr_i(ymat,tab,id,nrep,decorr.method)         ## decorr of obs
      yd$rep=NULL
  ...
  ##
  ## Jittering of pd/pde and management of pd <=0 or pd >= 1
...
auto_bin<-function(xvec,plot.opt,verbose=FALSE) {
  ...
  if(is.na(pmatch(plot.opt$bin.method,c("optimal","user"))) && length(unique(xvec))<=nbin) {
    ...
    if(!is.na(pmatch(plot.opt$bin.method,"width"))) {
      if(is.null(plot.opt$xlog)) plot.opt$xlog=F
    ...
    if(!is.na(pmatch(plot.opt$bin.method,"optimal"))) {
      ...
      xgrp<-factor(cut(xvec,bnds,include.lowest=T))
      xpl<-tapply(xvec,xgrp,mean)
    }
  }
...
plot_npde_scatter_dt_L = function(tab,variable="time",outcome="npde",bin=F,plot.opt=list(bin.method="width",
                                                                                         bin.number=NULL,
                                                                                         bin.breaks=NULL,
                                                                                         bin.extreme=NULL,
                                                                                         xlog=F),conf = 0.9,method_band="theo"){
  ...
  huron2 = tab[,list(N=length(id)),keyby=c(variable)]
  ...
  if(length(variable)==1){
    ...
    p4 = ggplot() +theme_bw() + scale_y_continuous(name=outcome)+
      ...
      scale_linetype_manual(values = c(2,1,2),name="Observed percentiles")+
    ...
    p4 = ggplot() + theme_bw() + scale_y_continuous(name=outcome)+
      ...
      scale_linetype_manual(name="Observed percentiles",values=c(2,1,2))+
      scale_fill_manual(name="Theoretical percentiles",values=c("blue","red"),labels=c("90% PI of \n5-95th percentiles","90% PI of median"));p4
  }
  return(p4)
}
...
plot_npde_scatter_dt_TTE = function(tab2,variable="OFT",outcome="npde",adj=T,method="noverall",conf=0.9,covariate="cat",p=ggplot(),var='prob'){
  tab<-copy(tab2)
  dist=ifelse(outcome%in%c("npd","npde"),"norm","unif")
  tab[,y:=factor(y)]
  sens = ifelse(var=="prob",1,-1)
  ...
  if(length(variable)==1){
    ...
    if(adj){
      p4 = p + 
        geom_point(data=th_fin,aes(x=get(variable[1]),y=(get(outcome)-z)*sens,col=outlier,shape=y)) +theme_bw(base_size = 18) + 
        geom_line(data=th_fin,aes(x=get(variable[1]),y=z-z),lty=2) +
        geom_ribbon(data=th_fin,aes(x=get(variable[1]),ymin=(lower-z)*sens,ymax=(upper-z)*sens,fill="blue"),alpha=0.4)+
    ...
    if(method=="overall"){
      tab[,c("ord.x","z","upper","lower"):=gg_qq(get(outcome),conf=conf,distribution = dist)$tab,by=eval(variable[1])]
    }else{
      tab[,c("ord.x","z","upper","lower"):=gg_qq(get(outcome),conf=conf,distribution = dist)$tab,by=eval(variable[2:length(variable)])]
    }
    ...
    if(adj){
      p4 = p + geom_point(data=th_fin,aes(x=get(variable[1]),y=(get(outcome)-z)*sens,col=outlier,shape=y)) +theme_bw() + scale_x_continuous(name=variable[1]) +
        ...
        geom_ribbon(data=th_fin,aes(x=get(variable[1]),ymin=(lower-z)*sens,ymax=(upper-z)*sens,fill="blue"),alpha=0.2) +
...
VPC_TTE = function(obs,sim,censoring_type,p=ggplot(),verbose=F,var="prob",bin=T,plot.opt){
  ...
  # if(bin){
  #   dat_km$old_time=dat_km$time
  #   binning = auto_bin(xvec = dat_km$old_time,plot.opt=plot.opt,verbose=T)
  #   if(is.null(plot.opt$bin.breaks)) breaks=bounds
  #   
  #   dat_km[[variable[1]]] = bounds
  #   sim_o[[variable[1]]] = bounds
  # }else{
  ...
  if(var=="prob"){
    ...
    p = ggplot()+
      geom_step(data=dat_km,aes(x=time,y=1-y))
  }
  ...
  if(var=="prob"){
    p2 = p + geom_step(data=a,aes(x=time,y=1-med),lty=2,lwd=1) +
      ...
      coord_cartesian(xlim=c(0,max(time_interval)))+
      scale_y_continuous(name="Probability of event",limits = c(0,1));p2
  }else{
    p2 = p + geom_step(data=a,aes(x=time,y=med),lty=2,lwd=1) +
      geom_ribbon(data=a,aes(x=time,ymin=lower,ymax=upper),alpha=0.4,
                  inherit.aes=FALSE,stat="stepribbon")+
      coord_cartesian(xlim=c(0,max(time_interval)))+
      scale_y_continuous(name="Survival",limits = c(0,1));p2
  }
...
VPC_L = function(obs=NULL,sim,p=ggplot(),breaks=c(1,10,100,1000)){
  ...
  p2 = p1 + geom_line(data=tmp_line,aes(x=time,y=value,group=type,col=type),lty=2,lwd=1) +
    ...
    scale_fill_manual(name="Predicted percentiles",values=c("blue","red","blue"),
                      labels=c("90% PI of 5th percentiles","90% PI of median","90% PI of 95th percentiles"))+
    scale_color_manual(name="Predicted percentiles",values=c("blue","red","blue"),
                       labels=c("90% PI of 5th percentiles","90% PI of median","90% PI of 95th percentiles")) #+ 
  ...
  if(!is.null(obs)){
    ...
    p2 = p2 + geom_line(data=b,aes(x=time,y=value,lty=variable))+
      scale_linetype_manual(name="Observed percentiles",values = c(1,2,2),
                            labels=c("median","5th percentiles","95th percentiles"))
...
format_extended_npde = function(dat){
  if("rep"%!in%names(dat)) dat[,rep:=1]
  dat[,id:=as.num(id)]
  dat[,time:=as.num(time)]
  dat[,rep:=as.num(rep)]
  return(dat[,.(rep,id,time,y)])
}

write_data = function(list_data,path,name="dat.txt",with_MlxR=F){
  
  if(is.list(list_data)&!is.data.table(list_data)){
    for(i in 1:length(list_data)){
      list_data[[i]]=data.frame(list_data[[i]])
      write.table(list_data[[i]],file=paste0(path,"/",name[i]),sep=";",quote = F,na = ".",row.names = F)
    }
  }else{
    write.table(list_data,file=paste0(path,"/",name),sep=";",quote = F,na = ".",row.names = F)
  }
