...
main_compute_npde = function(obs,sim,type_outcome="continuous",
  ...
  for(o in 1:noutcome){
    ...
    if(type_outcome[o]=="continuous"){
      ...
      if(dependancies[[o]]==0){
        npde_o = prediction_discrepancie(dat = obs_o,simulated_data = sim_full_o,ties = ties,censoring_type=censoring_type)
      }else{
...
main_compute_plot = function(res,type_outcome="continuous",options=NULL){
  ...
  for(o in 1:noutcome){
    ...
    #### Load options ####
    order_cat = paste0(sort(unique(c(npde_o$y,sim_o$y))))
    censoring_type="right"
    outcome="npd"
    variable="time"
    conf=0.9
    ...
    if(type_outcome[[o]]%in%c("continuous")){
      ...
      if(type=="scatter"){
        ...
        myplot = plot_npde_scatter_dt_TTE(npde_o,variable = variable,outcome = outcome,adj = T,conf = conf,var=var)
      }else if(type=="vpc"){
        
        myplot = VPC_TTE(npde_o,sim_o,censoring_type,var=var)[[1]]
      }
...
correlation_matrix<-function(obs_i,sim_i,nrep,decorr.method,verbose=FALSE) {
  # print(names(sim_i))
...
plot_npde_scatter_dt_TTE = function(tab2,variable="OFT",outcome="npde",adj=T,method="overall",conf=0.9,covariate="cat",p=ggplot(),var='prob'){
  tab<-tab2
  dist=ifelse(outcome%in%c("npd","npde"),"norm","unif")
  tab[,y:=factor(y)]
...
VPC_TTE = function(obs,sim,censoring_type,p=ggplot(),verbose=F,var="prob"){
