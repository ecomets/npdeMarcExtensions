...
#' 
#' @name show.NpdeData
#' @aliases show show,NpdeData-method show.NpdeData
#' @docType methods
#' @keywords methods
#' 
...
setMethod("read",
          ...
          function(object, name.simdata, header=TRUE, sep="", na.strings=c("NA","."), verbose=FALSE) {
            ow <- options("warn")
            options("warn"=-1)
            if(exists(name.simdata)) {
              if(verbose) cat("Using the object called",name.simdata,"in this R session as the data.\n")
              dat<-get(name.simdata)
            } else {
              if(verbose) cat("Reading data from file",name.simdata,"\n")
              if(missing(header)) {
                x1<-try(read.table(name.simdata,nrows=1))
              ...
              x1<-unlist(strsplit(as.character(dat[1,1]),";",fixed=TRUE))
              if(length(x1)>1) mysep<-";"
              if(mysep!="") dat<-read.table(name.simdata,na.strings=c(".","NA"),sep=mysep)
              if(!is.numeric(dat[1,1]))
                dat<-read.table(name.simdata,na.strings=c(".","NA"),header=TRUE,sep=mysep)
              if(!is.numeric(dat[1,1])) {
...
npdeSimData<-function(npde.data,name.simdata,header=TRUE,verbose=FALSE) {
  ...
  x<-read(x1,name.simdata=name.simdata,header=header,verbose=verbose)
