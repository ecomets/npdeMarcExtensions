...
# 	for(igrp in grp) {
# 		mat<-apply(sim.ypl$ypl[sim.ypl$grp==igrp,3:dim(sim.ypl)[2]],2,quantile,quant) # ECO TODO check 3:dim() (needed ?)
# 		xtab<-apply(mat,1,quantile,quant.pi)
...
compute.bands.true<-function(grp,sim.ypl,quant=c(0.025,0.5,0.975), alpha=0.95) {
	# Compute a prediction interval around selected quantiles of the normal or uniform distribution, using simulated data (nsim columns in sim.ypl) over the values given by the grouping factor grp
	quant.pi<-c((1-alpha)/2,0.5,1-(1-alpha)/2)
	ugrp<-unique(grp)
	binf<-bsup<-bmed<-matrix(nrow=length(ugrp),ncol=length(quant), dimnames=list(ugrp,quant))
	for(igrp in ugrp) {
		mat<-apply(sim.ypl[grp==igrp,],2,quantile,quant)
		xtab<-apply(mat,1,quantile,quant.pi)
...
#' 
#' @return a list with 2 elements, xgrp (the number of the bin associated with each element of xvec) and xat (a named vector containing the mean of the elements of xvec contained in each bin; the name of the bin is the interval).
